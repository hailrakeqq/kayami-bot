...
const { translit, embed } = require('../utils')
...
module.exports = {
    ...
    async execute(message) {
        ...
        if (content.match(new RegExp(`^${prefix}([a-zA-Z])`, 'g'))) {
            ...
            const command = translit(args.shift().toLowerCase())
            ...
            try {
                ...
                if (cmd.admin && !guild.members.cache.get(author.id).permissions.has("ADMINISTRATOR")) return embed(message, `0xa74949`, `**У вас недостаточно прав на использование данной команды!**`, null, 15)
