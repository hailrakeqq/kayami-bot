...
module.exports = {
    ...
    async execute(message, client, args) {
        const url = `https://g.tenor.com/v1/random?q=animecuddle&key=${config.tenor}`
        const response = await fetch(url)
            .then(async response => {
                ...
                if (args[1] = '' || mb.id == message.author.id) {
                    message.reply('Ошибка, нужно тегнуть к кому вы хотите прижаться')
                } else {
                    ...
                    const embed = new MessageEmbed()
                        ...
                        .setTitle(`${message.member.user.username} прижался к ${mb.user.username}....`)
                        .setImage(gif)
