...
module.exports = {
    name: "lick",
    desc: "облизать пользователя",
    use: "lick <@user>",
    category: ":people_with_bunny_ears_partying: Взаимодействия",
    aliases: [],

    async execute(message, client, args) {
        const url = `https://g.tenor.com/v1/random?q=animelick&key=${config.tenor}`
        const response = await fetch(url)
            .then(async response => {
                ...
                if (args[1] = '' || mb.id == message.author.id) {
                    message.reply('Ошибка, нужно тегнуть кого вы хотите облизать')
                } else {
                    ...
                    const embed = new MessageEmbed()
                        ...
                        .setTitle(`${message.member.user.username} облизывает ${mb.user.username}....♡`)
                        .setImage(gif)
