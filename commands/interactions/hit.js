...
module.exports = {
    name: "hit",
    desc: "ударить пользователя",
    use: "hit <@user>",
    category: ":people_with_bunny_ears_partying: Взаимодействия",
    aliases: [],

    async execute(message, client, args) {
        const url = `https://g.tenor.com/v1/random?q=animepunch&key=${config.tenor}`
        const response = await fetch(url)
            .then(async response => {
                ...
                if (args[1] = '' || mb.id == message.author.id) {
                    message.reply('Ошибка, нужно тегнуть кого ударить')
                } else {
                    ...
                    const embed = new MessageEmbed()
                        ...
                        .setTitle(`${message.member.user.username} ударил ${mb.user.username}`)
                        .setImage(gif)
