...
module.exports = {
    name: "wink",
    desc: "подмигнуть пользователю",
    use: "wink",
    category: ":people_with_bunny_ears_partying: Взаимодействия",
    aliases: [],

    async execute(message, client, args) {
        const url = `https://g.tenor.com/v1/random?q=animewink&key=${config.tenor}`
        const response = await fetch(url)
            .then(async response => {
                ...
                if (args[1] = '' || mb.id == message.author.id) {
                    message.reply('Ошибка, нужно тегнуть кому вы хотите подмигнуть')
                } else {
                    ...
                    const embed = new MessageEmbed()
                        ...
                        .setTitle(`${message.member.user.username} подмигнул(-а) ${mb.user.username}  (･ω<)`)
                        .setImage(gif)
